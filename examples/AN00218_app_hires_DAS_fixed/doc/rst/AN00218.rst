...
To start using the high resolution delay lines, you need to add ``lib_mic_array`` to you Makefile::
...
An PDM microphone requires a clock and a data pin. For eight PDM mocrophones a single clock can be shared
 between all microphones and the data can be sampled on a single 8 bit port. On an xCORE the pins are
controlled by ``ports``. The application therefore declares one 1-bit port and one 8-bit port:
...
together and to the application (``hires_DAS_fixed()``). There needs to be one ``decimate_to_pcm_4ch()`` task per
four channels that need processing. The needs to be one ``hires_delay`` task per eight channels.
...
be delayed by up to ``HIRES_MAX_DELAY`` (default 256) samples at a sample rate of 384kHz. Setting of the taps
