...
void app_i2s_send(ma_interface_t* app_data,
                  ...
                  int32_t* samples)
{
  int32_t frame[MIC_ARRAY_CONFIG_MIC_COUNT] = {0};
  if(app_data->override)
  {
    ma_frame_rx(frame, app_data->c_from_mic_array, 1, 1);
  }
  else
  {
...
void app_i2s_init(ma_interface_t* app_data,
                  i2s_config_t* config)
{
  static unsigned sample_rate = 16000; // dac3101_configure() in lib_board_support, xk_voice_l71.xc only allows these 2
  app_data->override = app_data->override ^ 1;
  printf("Starting at sample rate %u, 1mic override %u\n", sample_rate, app_data->override);
  ...
  xk_voice_l71_AudioHwConfig(&hw_config, sample_rate, MCLK_48);

  chan_out_word(app_data->c_from_mic_array, sample_rate); // convey samp freq to app_mic()
  chan_out_word(app_data->c_from_mic_array, app_data->override);
