...
This demo application shows a simple Delay and Sum (DAS) bemformer. 
...
The PDM microphones interface task and the decimators have to be connected 
together and to the application (``lores_DAS_fixed()``). There needs to be one 
...
decimators via two streaming channels. Finally, the decimators have to be connected to 
the application.

.. literalinclude:: app_lores_DAS_fixed.xc
   :start-on: streaming chan c_pdm_to_lores[2];
...
.. literalinclude:: app_lores_DAS_fixed.xc
   :start-on: int data[8]
   :end-before: void lores_DAS_fixed
