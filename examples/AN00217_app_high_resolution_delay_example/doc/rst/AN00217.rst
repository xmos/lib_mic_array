...
The application uses a many logical cores running in parallel in order to implement all the tasks required
to capture the PDM audio, process it and transfer it to an application for further processing.


How to use high resolution delays with ``lib_mic_array``
--------------------------------------------------------
...
that interfaces to the PDM microphones, decimates the PDM and finally packs it into frames.

To start using the microphone array library you need to add ``lib_mic_array`` to
your ``Makefile``::
...
four channels that need processing. There needs to be one ``hires_delay`` task per eight channels.
...
The return value of the above is a pointer to the frame that the application (``hires_DAS_fixed()``) is allowed 
...
active delay lines tap positions.
