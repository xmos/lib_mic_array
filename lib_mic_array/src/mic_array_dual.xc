...
static inline int dc_eliminate(int x, int *prev_x, long long *state){
#define S 0
#define N 8
    long long X = x;
...
void mic_dual_pdm_rx_decimate(buffered in port:32 p_pdm_mic, streaming chanend c_2x_pdm_mic, streaming chanend c_ref_audio[]){
  ...
  int [[aligned(8)]] mid_stage_delay[MIC_DUAL_NUM_CHANNELS][16 * 2] = {{0}}; //Double length for circular buffer simulation
  ...
  const int * unsafe phase_coeff_ptrs[6];
...
//#pragma loop unroll
    ...
    if (final_stage_phase == 6) unsafe{
      ...
      if (MIC_DUAL_NUM_REF_CHANNELS > 0 && !isnull(c_ref_audio)) {
      #if MIC_DUAL_NUM_REF_CHANNELS > 0
      select{
