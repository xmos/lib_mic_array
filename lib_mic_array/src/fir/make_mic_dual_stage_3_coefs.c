...
int main(void)
{
    ...
    fprintf(h_file, "// Copy" "right 2020-%s XMOS LIMITED.\n", year);
    ...
    for (int df = 2; df <= MAX_DECIMATION_FACTOR; df += 2) {
        ...
        fprintf(c_file, "static mic_dual_third_stage_coef_t g_third_stage_div_%d_fir_dual_i[%d] __attribute__((aligned(8))) = {\n", df, df);
        fprintf(h_file, "extern mic_dual_third_stage_coef_t * const g_third_stage_div_%d_fir_dual;\n", df);
        ...
        fprintf(c_file, "};\n");
        fprintf(c_file, "mic_dual_third_stage_coef_t * const g_third_stage_div_%d_fir_dual = g_third_stage_div_%d_fir_dual_i;\n\n", df, df);
    }
