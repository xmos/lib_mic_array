    ...
    while(1){
        ...
        for(unsigned i=0;i<4;i++){
            for(unsigned j=0;j<(1<<FRAME_SIZE_LOG2);j++){
                current->data[i][j].re<<=6;
                current->data[i][j].im<<=6;
            }

            lib_dsp_fft_forward_complex((lib_dsp_fft_complex_t*)current->data[i], (1<<FRAME_SIZE_LOG2), lib_dsp_sine_512);
        ...
        select {
            ...
            default:{
                ...
                unsigned best =0;
                for(unsigned i=0;i<256;i++){
                    uint64_t mag = (uint64_t)current->data[0][i].re * (uint64_t)current->data[0][i].re +
                            (uint64_t)current->data[0][i].im * (uint64_t)current->data[0][i].im;
