...
class Test_BasicMicArray(object):
  ...
  def test_BasicMicArray(self, request, chans, frame_size, use_isr):
    ...
    xe_path = f'{cwd}/bin/{cfg}/tests-signal-pdmrxisr_{cfg}.xe'
    ...
    print(f"filter.s2.DecimationFactor = {filter.s2.DecimationFactor}")
    ...
    print(f"frames = {frames}, samp_total = {samp_total}")
